...
def test_searchengine_elastic_cluster_health(host):
    out = host.check_output('curl  -L -k -u "elastic:elastic_password" https://127.0.0.1:9200/_cluster/health?pretty')
...
def test_searchengine_search_elastic_connection(host):
    out = host.check_output('curl -L -k -u  "elastic:elastic_password" https://127.0.0.1:9200/image_keyvalue_pair_metadata')
