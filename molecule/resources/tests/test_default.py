...
def test_searchengine_elastic_cluster_health(host):
    out = host.check_output('curl  -k -u "elastic:elastic_password" https://0.0.0.0:9201/_cluster/health?pretty')
...
def test_searchengine_search_elastic_connection(host):
    out = host.check_output('curl -k -u  "elastic:elastic_password" https://0.0.0.0:9201/image_keyvalue_pair_metadata')
